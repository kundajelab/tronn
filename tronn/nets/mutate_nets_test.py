...
class MutagenizerTests(tf.test.TestCase):
    ...
    def test_preprocess_for_shuffle_mut(self):
    ...
    def test_mutagenize_multiple(self):
        ...
        assert left_unchanged, "left is not unchanged {}".format(
            np.where(np.equal(example[:,:445], mut_example[:,:445]))[1])
        ...
        assert left_unchanged, "left is not unchanged {}".format(
            np.where(np.equal(example[:,:470], mut_example[:,:470]))[0])
