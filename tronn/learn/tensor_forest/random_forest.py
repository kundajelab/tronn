...
def get_model_fn(params,
  ...
  def _model_fn(features, labels, mode):
    ...
    logits_tmp = graph_builder.inference_graph(features)
    ...
    task_logits_list = array_ops.unstack(logits_tmp, axis=1)
    task_logits_transformed = []

    logits = logits_tmp
    
    # DK edits here
    if False:
      for task_idx in xrange(len(task_logits_list)):
        task_logits = math_ops.log(
          math_ops.maximum(task_logits_list[task_idx] / math_ops.maximum(
            1.0 - task_logits_list[task_idx], EPSILON), EPSILON))
        task_logits_transformed.append(task_logits)
      logits = array_ops.stack(task_logits_transformed, axis=1)
    
    if not params.regression and params.num_classes == 2:
      class_1_probs = array_ops.slice(logits_tmp, [0, 1], [-1, 1])
      logits = math_ops.log(
