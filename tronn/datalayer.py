...
def hdf5_to_slices(hdf5_file, batch_size, tasks=[]):
...
def load_data_from_filename_list(hdf5_files, batch_size, tasks=[], shuffle_seed=0):
    print 'loading data for tasks %s from %s' % (tasks, hdf5_files)
    example_slices_list = [hdf5_to_slices(hdf5_file, batch_size, tasks) for hdf5_file in hdf5_files]
