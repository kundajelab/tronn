...
def parse_args():
    ...
    parser.add_argument(
        "--reduce_type", default="any",
        help="type of reduction across the inference tasks (any or all)")
...
def main():
    ...
    print args.inference["targets"][0]
    ...
    # get background subset. if multiple selection strings,
    ...
    print background_pwm_names # TODO check foregrounds to make sure they match
    ...
    pvals = np.ones([len(foregrounds)] + list(background_data[args.scores_key].shape[1:]))
    ...
    for foreground_idx in range(len(foregrounds)):
        ...
        logging.info("loading foreground {} with {}".format(foreground[0], foreground[1]))

        # setup data loader
        # TODO is it worth saving out a new dataset json?
        ...
        # now you have foreground and background,
        # iterate through tasks
        # for each task, get positive background (closest match) and run differential test
        for task_idx in range(foreground_data[args.scores_key].shape[1]):
            logging.info("test diff motifs for task idx {}".format(task_idx))

            # get foreground for task
            task_foreground_scores = foreground_data[args.scores_key][:,task_idx]
            
            # select specific background for task
            task_background_indices = np.where(
                inference_targets[:,task_idx] > 0)[0]
            task_background_scores = background_data[args.scores_key][
                task_background_indices][:,task_idx]
            task_background_gc = background_data[args.gc_key][task_background_indices]

            # run tests
            pvals[foreground_idx, task_idx] = test_differential_motifs(
                task_foreground_scores,
                task_background_scores,
                foreground_data[args.gc_key],
                task_background_gc)

            # run hits version
            hits_pvals[foreground_idx, task_idx] = test_differential_motifs(
                task_foreground_scores != 0,
                task_background_scores != 0,
                foreground_data[args.gc_key],
                task_background_gc)

    # calc qval thresholds
    scores_qval_thresholded = threshold_by_qvalues(pvals, qval_thresh=0.05)
    hits_qval_thresholded = threshold_by_qvalues(hits_pvals, qval_thresh=0.05)

    # overall threshold
    overall_thresholded = np.multiply(
        scores_qval_thresholded,
        hits_qval_thresholded)
    if args.reduce_type == "any":
        overall_thresholded = np.any(overall_thresholded, axis=1)
    
    # and save out
    _RAW_PVALS_GROUP = "pvals"
    score_pvals_key = "score.pvals"
    hits_pvals_key = "hits.pvals"
    sig_key = "sig"

    results = {
        score_pvals_key: pvals,
        hits_pvals_key: hits_pvals,
        sig_key: overall_thresholded}
    
    pvals_file = "{}/pwms.differential.pvals.h5".format(
        args.out_dir)
